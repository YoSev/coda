...
func resolveString(input string, codaJSON []byte) any {
	// Updated regex pattern: support spaces before and after the pipe symbol
...
func parseFilters(raw string) []Filter {
	...
	// Split on pipes, and trim spaces around the pipe
	parts := strings.Split(raw, "|")
	for _, part := range parts[1:] { // skip the first part (path)
		part = strings.TrimSpace(part)
		if part == "" {
			continue
		}
		// Split filter name and argument (e.g., "join" or "join:/")
